...
ExplodingSprite::ExplodingSprite(const Sprite& s) :
  ...
  frames() {
  makeChunks(
    Gamedata::getInstance().getXmlInt(s.getName()+"/chunksize")
  );
}

ExplodingSprite::ExplodingSprite(const std::string& n,const Vector2f& pos, const Vector2f& vel,const Frame* fram) :
  Sprite(n, pos, vel, fram),
  name(n),
  chunks(),
  freeList(),
  frames() {
...
void ExplodingSprite::draw() const { 
  ...
  while (ptr != chunks.end()) {
    //std::cout << "in draw exploding sprite" << std::endl;
    ptr->draw();
    ++ptr;
  }
}
...
void ExplodingSprite::makeChunks(unsigned int n) { 
  ...
  //std::cout << "in make chunks" << std::endl;
