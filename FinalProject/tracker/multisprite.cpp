...
void MultiSprite::advanceFrame(Uint32 ticks) {
  //std::cout << "in multisprite update" <<std::endl;
...
MultiSprite::MultiSprite( const std::string& name) :
  ...
  Drawable(name, 
           Vector2f((rand()%2?1:-1)* getRand(10, Gamedata::getInstance().getXmlInt(name+"/srcX")), 
...
MultiSprite::MultiSprite(const std::string& name, float x, float y, float sX, float sY) :
